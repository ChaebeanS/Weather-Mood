...
function applyWeather(weather) {
    ...
    // 숫자만 넣기 (mm는 HTML에 이미 있음)
    document.getElementById("rain").childNodes[0].nodeValue = rainValue;
...
function getPhaseByAge(lunAge) {
    ...
    if (lunAge < 14)
        return { eng: "waxing gibbous", kor: "상현망간" };

    if (lunAge < 17)
        return { eng: "full moon", kor: "보름달" };

    if (lunAge < 22)
        return { eng: "waning gibbous", kor: "하현망간" };
